...
function [gridDataStruWrapper] = createDataIndexing( points,basicSideLength,basicPointsNumInACell,maxCellsNum)
    if (nargin<2)
    ...
    disp('     layer :1');
    layers(1)=getLayer(points,basicSideLength,basicPointsNumInACell,[],1,gridIndexOfEachPoint,nonEmptyCell,nonEmptyCellNum,pointsInCell);
    %gridDataStruWrapper=layers;
    %return;
    count=1;
    
    while 1
        grids=floor(nonEmptyCell/2);
        [mapKeys,gridIndexOfEachPoint,nonEmptyCell,pointsInCell,nonEmptyCellNum,pointsInCellNumArr]=map2str(grids);
        if  nonEmptyCellNum>maxCellsNum
            count=count+1;
            disp(strcat('     layer :',num2str(count)));
            aLayer=getLayer(points,basicSideLength,basicPointsNumInACell,layers(count-1),count,gridIndexOfEachPoint,nonEmptyCell,nonEmptyCellNum,pointsInCell);
            layers(count)=aLayer;
        else
            break;
        end
...
function [layerStruct]= getLayer(points,basicSideLength,basicPointsNumInACell,childLayerStruct,layerId,gridIndexOfEachCell,nonEmptyCell,nonEmptyCellNum,childrenCells)
    
    %Layer structure 
    layerStruct.layer=layerId;
    layerStruct.cellSideLength=basicSideLength*2^(layerId-1);
