...
for i=1:n
    if visited(i)
        ...
        if(numel(sortLoc)>MinPts)  %%%2Eps内点数大于MinPts
            if(sortDists(MinPts)<radius)%%%核心点判断
                C=C+1;IDX(i)=C;
                visited(i)=true;
    ...
    filteredNoise(normNoise==1)=0;
    countnoise=sum(filteredNoise);
    disp(['rangeQuery:',num2str(evaluation(1)),'  NeighborsQuery:',num2str(evaluation(2)),'  filteredNoise:',num2str(countnoise),'  sum:',num2str(sum(evaluation)+countnoise),'  ',num2str(sum(visited))])
    ...
    function [Neighbors]=ExpandCluster(sortLoc,sortDists,radius)
        ...
        while count<length(sortLoc)
            %disp([evaluation,countnoise,sum(evaluation)+countnoise,sum(visited)]);
